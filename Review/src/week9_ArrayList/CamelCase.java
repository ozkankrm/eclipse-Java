...
public class CamelCase {
	...
	public static ArrayList<String> makeCamelCase (ArrayList<String> words) {
		...
		for(String word : words) {  // java is fun
			...
			// arraye çevirip önce lowercase sonrada her word kendi içinde split ile array yaptýk
			String [] eachWord = word.toLowerCase().split(" "); // [java, is, fun]
			
			// ilk element lowercase ile baþlar ve bunu burda declare ettik 
			String camelWord = eachWord[0]; // java
			
			// i=1 her wordun ikinci elementinden baþlattýk
			for(int i = 1; i < eachWord.length; i++) {
				
				// her elementin ilk harfini uppor yaptýk
